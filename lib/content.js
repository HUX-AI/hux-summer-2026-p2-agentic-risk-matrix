...
export function getReport() {
  const { _comment, ...report } = read('report.json');
  return report;
}

export function getScenarios() {
  const { _comment, ...scenarios } = read('scenarios.json');
  return scenarios;
}

export function getAssessment() {
  const { _comment, ...assessment } = read('assessment.json');
  return assessment;
}
...
/**
 * Sections keyed by slug. app/page.js places each one by name rather than
 * rendering them in a block, because the prose is interleaved with the matrix,
 * the taxonomy and the instruments rather than sitting in one run.
 */
export function getSectionsBySlug() {
  return Object.fromEntries(getSections().map((s) => [s.slug.replace(/^\d+-/, ''), s]));
}
